...
def getValores(origenes, destinos):
    ...
    for x in range(origenes):
        for y in range(destinos):
            valores[x][y] = verifyIntInput(f'Introducir el valor de la fila {x} en la columna {y}\n')
    return valores

def getOferta(num):
    oferta = []
    for x in range(num):
        aux = verifyIntInput(f'Introducir la oferta de la Fila {x}\n')
        oferta.append(aux)
    return oferta
...
def getDemandas(num):
    demanda = []
    for x in range(num):
        aux = verifyIntInput(f'Introducir la demanda de la columna {x}\n')
        demanda.append(aux)
    return demanda
...
def getDestinos():
    while True:
        aux = verifyIntInput("Introducir el numero de Columnas\n")
        if (aux > 1):
            break
...
def getOrigenes():
    while True:
        aux = verifyIntInput("Introducir el numero de filas\n")
        if (aux > 1):
            break
        print('escribir un numero igual o mayor a dos')
    return aux
